  ...
  g.V().has('Metadata.Type.Person.Organisation', eq('Person.Organisation'))
    ...
    .each { metric ->
      metric.each { key, metricvalue ->
        ...
        sb.append(" {\"dsar_source_type\":\"${key['dsar_type']}#${key['dsar_status']}#${key['dsar_source_type'].label().toString().replaceAll('[_|.]',' ')}#${key['dsar_age']}\", \"dsar_source_name\":\"${key['dsar_source_name']}\", \"num_dsars\": $metricvalue }")
